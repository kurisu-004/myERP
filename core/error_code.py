...
class ErrCode(IntEnum):
    ...
    BIZ_PART_BATCH_INVALID_QUANTITY = (
        20111  # 2026-07-29：拆分/部分流转数量非法（≤0 或超过批次量）
    )
    ...
    BIZ_WORKER_HOLD_LIMIT_EXCEEDED = (
        20204  # 2026-08-05：工种 max_held_batches 上限触顶 → 拒领取
    )
    ...
    BIZ_ASSEMBLY_TOO_MANY_CHILDREN = (
        20303  # 子件 > 99，序列号 {serial}-{i:02d} 派生失败
    )
    ...
    BIZ_PART_FILE_DUPLICATE = (
        21108  # 2026-07-14：同 part+kind+content_sha256 撞唯一索引（并发兜底）
    )
    ...
    BIZ_DELIVERY_TEMPLATE_TOO_MANY_PARTS = (
        21112  # 2026-07-20：所选零件超过模板数据行容量（法 14 / 路 25）
    )
    BIZ_DELIVERY_PRINT_BAD_ORDER = (
        21113  # 2026-08-02：custom_order 含非法 batch id 或漏行（422）
    )
    ...
    BIZ_SHELF_NO_MATCH_FOR_PROCESS = (
        20506  # 没有 active 货架映射指定 process → RETURN picker 无候选
    )
    BIZ_SHELF_PROCESS_NOT_MAPPED = (
        20507  # 货架未映射该工序（place/release/receive/complete_repair 422）
    )

    # ---- 账号（t_user / t_user_role）----
    # 206xx：账号相关
    # 2026-09-19 IAM 域迁出：账号 / 角色相关校验由 backend-rust v2 的
    # `/api/v2/iam/*` 承接；本仓已删除 service/auth.py / service/user.py /
    # service/menu.py + 对应 repository / model。错误码 BIZ_USER_DUPLICATE_USERNAME
    # (20602) / BIZ_USER_NO_ROLE (20606) 随抛点一并删除；BIZ_USER_ACCOUNT_NOT_FOUND
    # / BIZ_USER_INACTIVE / BIZ_USER_ROLE_DUPLICATE / BIZ_USER_ROLE_NOT_FOUND 暂留
    # 作为未来 rust 端的对齐锚点（本仓当前无引用；2026-Q4 rust 端 IAM 上线后
    # 由 rust 侧定义具体取值，本仓对齐再删除）。
    ...
    BIZ_AUTH_REFRESH_INVALID = (
        40103  # 401  refresh token 失效 / 版本不匹配 / 用户已停用
    )
    BIZ_AUTH_OLD_PASSWORD_MISMATCH = 40104  # 401  修改密码时旧密码错误
    BIZ_AUTH_SHELF_MISMATCH = (
        40301  # 403  SHELF_ACCOUNT 账号操作的 shelf 与 JWT 中不一致
    )
    ...
    BIZ_CUSTOMER_IN_USE = (
        20109  # 还有 active 子节点 / 被 part 或 assembly 引用 → 拒软删
    )
    ...
    BIZ_OUTSOURCE_COMPANY_BAD_PROCESS = (
        21203  # 工序不存在 / 不是 OUTSOURCE/INHOUSE 类别
    )
