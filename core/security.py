...
- `tests/test_delivery_note_print_api.py` — 调用 `create_access_token` /
  ...
  / `ErrCode.BIZ_AUTH_TOKEN_EXPIRED`）；本仓**唯一**真正 import
  `core.security` 的下游
- `tests/unit/test_part_service_workflow.py` — **不 import core.security**，
  只断言扫码流程中 SHELF_ACCOUNT 操作货架不匹配时抛
  `ErrCode.BIZ_AUTH_SHELF_MISMATCH`（仅消费 ErrCode 枚举）
- `core/permission.py` docstring — 历史设计说明，不实际 import

彻底下线本模块需先迁移 `tests/test_delivery_note_print_api.py` 到「不依赖
真实 JWT 签发 / 解码」的纯 mock 路径（参考 `test_part_*_filter.py` 系列
直接 patch `get_current_user` 的 fixture 写法）。`test_part_service_workflow.py`
无需迁移（不 import 本模块）；在迁移完成前，本模块保留原逻辑。
"""
