<!--
  DeliveryNoteNew.vue — 文员生成送货单（PR-B 2026-07-10）

  - 默认拉 status=READY_TO_SHIP 的零件列表（按 planned_delivery_date 升序）
  - 支持客户筛选（与 PartsList 同款 el-tree-select popover）
  ...
  <div class="delivery-note-new">
    ...
    <div class="sheet-wrapper">
      ...
      >
        ...
        <el-table-column prop="serial_no" label="流水号" width="110" show-overflow-tooltip>
...
function onSelectAll(): void {
  // el-table 默认全选仅本页；这里把全部 items 视为已选
...
async function onGenerate(): Promise<void> {
  ...
  try {
    await ElMessageBox.confirm(
      `将按选中 ${selectedIds.value.length} 件零件生成送货单 Excel，确认继续？`,
      '生成送货单',
